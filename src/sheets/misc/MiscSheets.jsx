...
export function CalendarAddLogisticSheet({showId,event}){
  ...
  return <><Field label="Type"><div className="seg" id="al-kind">{[['travel','Flight / transfer'],['stay','Hotel']].map(([k,l],i)=><button type="button" data-v={k} className={i===0?'on':''} key={k} onClick={x=>{call('segPick',x.currentTarget);call('toggleLogisticAddFields')}}>{l}</button>)}</div></Field><div id="al-travel-fields"><Field label="Travel mode"><div className="seg" id="al-icon">{[['plane','Flight'],['car','Ground'],['ferry','Ferry'],['train','Train'],['bus','Coach'],['walk','Walk'],['cycle','Cycle']].map(([k,l],i)=><button type="button" data-v={k} className={i===0?'on':''} key={k} onClick={x=>{call('segPick',x.currentTarget);call('toggleLogisticAddFields')}}>{l}</button>)}</div></Field><Field label="Service number (optional)" id="al-code" placeholder="KL1008 / train no."/><div className="row-2"><Field label="From" id="al-from" placeholder="Departure"/><Field label="To" id="al-to" placeholder="Arrival"/></div><div className="row-2"><Field label="Departure" id="al-start" type="time"/><Field label="Arrival" id="al-end" type="time"/></div><div id="al-driver-name-wrap" style={{display:'none'}}><Field label="Operator / driver" id="al-driver-name" placeholder="e.g. Marco · Uber"/></div></div><div id="al-stay-fields" style={{display:'none'}}><Field label="Hotel name" id="al-place" placeholder="e.g. Hilton"/><Field label="Address" id="al-addr" placeholder="Street, city"/><Field label="Check-in time" id="al-checkin" type="time"/></div><Field label="Date" id="al-date" type="date" value={e.date}/><button className="btn" onClick={()=>call('saveLogisticFor',showId)}>Add</button><Spacer/></>;
}
