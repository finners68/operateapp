...
export function CalendarAddLogisticSheet({showId,event,presetKind,presetIcon,lockedMode}){
  ...
  return <>
    ...
    <div id="al-travel-fields" style={stay?{display:'none'}:undefined}>
      ...
      <div id="al-driver-name-wrap" style={{display:showDriver?'':'none'}}>
        <Field label="Driver name" id="al-driver-name" placeholder="e.g. John Smith"/>
        <Field label="Company / operator (optional)" id="al-operator" placeholder="e.g. Parklife Ground Transport"/>
      </div>
    </div>
    <div id="al-stay-fields" style={stay?undefined:{display:'none'}}>
      <Field label="Accommodation name" id="al-place" placeholder="e.g. Hilton"/>
      <Field label="Address" id="al-addr" placeholder="Street, city"/>
      <Field label="Check-in time" id="al-checkin" type="time"/>
    </div>
    <Field label="Date" id="al-date" type="date" value={e.date}/>
    <button className="btn" onClick={()=>call('saveLogisticFor',showId)}>Add</button>
    <Spacer/>
  </>;
}
export function CalendarItemSheet({id,item}){
  const e=item||events().find(x=>x.id===id)||{},travel=e.kind==='travel',stay=e.kind==='stay',icons=[['plane','Flight'],['car','Ground'],['ferry','Ferry'],['train','Train'],['bus','Coach'],['walk','Walk'],['cycle','Cycle']];
  return <><Field label="Date" id="it-date" type="date" value={e.date}/>{travel?<><Field label="Travel mode"><div className="seg" id="it-icon">{icons.map(([k,l])=><button type="button" data-v={k} className={(e.icon||'plane')===k?'on':''} key={k} onClick={x=>call('segPick',x.currentTarget)}>{l}</button>)}</div></Field><Field label="Service number (optional)" id="it-code" value={e.flightNo || e.trainNo || e.ferryNo || e.coachNo} placeholder="KL1008 / train no."/>{(e.icon||'plane')==='car'?<><Field label="Driver name" id="it-driver-name" value={e.driverName} placeholder="John Smith"/><Field label="Company / operator (optional)" id="it-operator" value={e.operator && e.operator!==e.driverName?e.operator:''} placeholder="Parklife Ground Transport"/></>:null}<div className="row-2"><Field label="From" id="it-from" value={e.fromName || e.from} placeholder="Departure"/><Field label="To" id="it-to" value={e.toName || e.to} placeholder="Arrival"/></div><div className="row-2"><Field label="Departure" id="it-start" type="time" value={e.start}/><Field label="Arrival" id="it-end" type="time" value={e.end}/></div>{(e.icon||'plane')==='car'?<><Field label="Driver phone" id="it-phone" type="tel" value={e.phone}/><Field label="WhatsApp (if different)" id="it-wa" type="tel" value={e.whatsapp}/></>:null}</>:null}{stay?<><Field label="Accommodation name" id="it-place" value={e.place} placeholder="Hilton"/><Field label="Address" id="it-addr" value={e.addr} placeholder="Address"/><Field label="Check-in" id="it-info" value={(e.info||'').replace(/^Check-in:\s*/i,'')} placeholder="14:00"/><Field label="Booking reference" id="it-ref" value={e.bookingRef} placeholder="Confirmation number"/></>:null}<button className="btn" onClick={()=>call('saveItem',id)}>Save</button><Spacer/><button className="btn danger" onClick={()=>call('delItem',id)}><Icon name="trash" size={15}/> Delete</button><Spacer/></>;
}
